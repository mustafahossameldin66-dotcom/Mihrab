...
let state={theme:'mono',lang:'ar',view:'home',dayType:'كلية',todayDate:'',today:{},plan:{},weekly:{marketingHours:0,mckinsey:false,dose:false,review:false,rating:'',cert:false},weekDayTypes:{},quranFrameOpen:false,mode:'normal',modeDate:'',schemaVersion:5,settings:{lowPower:false},inbox:[],library:[],backupAt:'',metrics:{focusMinutes:0,sessions:0},history:{events:[],series:{},milestones:[],plans:{}},dailyMomentum:{date:'',index:0},faith:{sunsetMinutes:null,calendarSource:'tabular',prayerTimes:{fajr:'05:00',dhuhr:'12:00',asr:'15:30',maghrib:'18:00',isha:'19:30'},prayerLog:{},fastingLog:{},istighfar:{date:'',today:0,total:0},obligations:[],proofVault:[]}};
...
function taskHTML(items){return '<div class="tasks">'+items.map(([id,text])=>{const done=tChecked(id);const badge=taskBadge(id);const label=(state.lang==='en'&&TASK_EN[id])?TASK_EN[id]:text;return `<div class="task-item ${done?'done':''}" data-task-id="${esc(id)}"><input type="checkbox" id="task_${esc(id)}" data-today-id="${esc(id)}" ${done?'checked':''}><label class="task-text" for="task_${esc(id)}">${esc(label)}</label>${badge?`<span class="mihrab-badge badge-${badge[1]}">${esc(badge[0])}</span>`:''}</div>`}).join('')+'</div>'}
// Public render helpers: the page renderers are defined before the runtime IIFE.
// Keep these helpers in the same (global) scope so Home can render without reaching
// into the IIFE's private lexical scope. The IIFE keeps its optimized private copy.
function activeMode(){
  return state.modeDate===keyDate() ? state.mode : (dayTypeToMode[todayDayType()]||'normal');
}
function taskObjects(){
  const items=dayTasks(todayName()).filter(([id])=>{
    ...
    return true;
  }).map(([id,text])=>({id,label:text,duration:durationFor[id]||15,priority:priorityOf(id),done:tChecked(id)}));
...
function dailyMomentum(){
 ...
 const messages=state.lang==='en'?[
  ...
  'اختار أهم خطوة جاية، مش كل الخطوات مرة واحدة.',
...
function nextPrayerInfo(){
 const now=new Date(), map=[['fajr','الفجر','Fajr'],['dhuhr','الظهر','Dhuhr'],['asr','العصر','Asr'],['maghrib','المغرب','Maghrib'],['isha','العشاء','Isha']];
 for(const [id,label,enLabel] of map){const [h,m]=String(state.faith?.prayerTimes?.[id]||'00:00').split(':').map(Number);const d=new Date(now);d.setHours(Number.isFinite(h)?h:0,Number.isFinite(m)?m:0,0,0);if(d>now)return {id,label,enLabel,time:state.faith.prayerTimes[id],date:d};}
 const [h,m]=String(state.faith?.prayerTimes?.fajr||'05:00').split(':').map(Number);const d=new Date(now);d.setDate(d.getDate()+1);d.setHours(Number.isFinite(h)?h:5,Number.isFinite(m)?m:0,0,0);return {id:'fajr',label:'الفجر',enLabel:'Fajr',time:state.faith.prayerTimes.fajr,date:d,tomorrow:true};
}
function renderHome(){
 ...
 const rooms=[
  ['marketing','✦',en?'Marketing':'التسويق',en?'Craft your professional edge.':'غرفة صناعة الأثر المهني.'],
  ['shari','◇',en?'Islamic Studies':'العلم الشرعي',en?'Roots, clarity, understanding.':'جذورك: علم ووعي وفهم.'],
  ['quran','۝',en?'Qur’an':'القرآن',en?'A quiet place to stay connected.':'موضع هادئ للصلة بالمحفوظ.'],
  ['courses','▱',en?'Courses':'الكورسات',en?'Supporting systems, kept in their place.':'أنظمة مساندة، بلا زحام.'],
  ['system','⚙',en?'System':'النظام',en?'Tune the world around you.':'اضبط طريقة تشغيل عالمك.'],
  ['home','⌂',en?'Today':'اليوم',en?'Your central chamber.':'الغرفة المركزية.']
 ];
 const T=en?{
 ...
 return `<div class="sanctuary-home">
   ...
   <section class="faith-pulse">
     <div class="faith-pulse-main"><span class="micro-label">04 / ${en?'SPIRITUAL PULSE':'نبض اليوم الشرعي'}</span><b>${esc(hijriLabel(spiritualSnapshot().h,en))}</b><small>${en?'Calculated calendar · the Islamic day follows your Maghrib setting.':'تقويم حسابي · اليوم الشرعي يتبع وقت المغرب الذي تضبطه.'}</small></div>
     <div class="faith-pulse-item"><span>${en?'Next prayer':'الصلاة التالية'}</span><b>${esc(en?nextPrayerInfo().enLabel:nextPrayerInfo().label)}</b><small>${esc(nextPrayerInfo().time)}${nextPrayerInfo().tomorrow?(en?' · tomorrow':' · غدًا'):''}</small></div>
     <div class="faith-pulse-item"><span>${en?'Istighfar today':'استغفار اليوم'}</span><b>${istighfarToday().today}</b><small>${en?'quiet tally':'عداد هادئ'}</small></div>
   </section>
   ...
   <section class="world-chamber">
     ...
     <div class="room-constellation">${rooms.map(([id,ic,title,desc],i)=>`<button class="room-node room-${i+1} ${id===state.view?'current':''}" onclick="navigate('${id}')"><span class="room-orbit"></span><i>${ic}</i><b>${esc(title)}</b><small>${esc(desc)}</small><em>${String(i+1).padStart(2,'0')}</em></button>`).join('')}</div>
   </section>

   <section class="execution-chamber" id="dailyTasks">
     <div class="execution-head"><div><span class="micro-label">03 / ${T.field}</span><h2>${en?'What is actually waiting for you.':'إيه اللي مستنيك فعلًا.'}</h2></div><div class="execution-status"><span>${done}/${total}</span><small>${T.completed}</small></div></div>
     ...
     <aside class="card sanctuary-side-panel"><div class="panel-glow"></div><span class="micro-label">04 / ${T.principles}</span><h3>${en?'Nothing sacred gets crowded out.':'الحاجات الأهم ما تتزاحمش.'}</h3><div class="principle-stack"><div><b>01</b><span>${en?'Reviews before novelty.':'المراجعة قبل الجديد.'}</span></div><div><b>02</b><span>${en?'Qur’an & Islamic studies stay protected.':'القرآن والشرعي يفضلوا ثابتين.'}</span></div><div><b>03</b><span>${en?'Supporting systems shrink first.':'الأنظمة المساندة تتقلص أولًا.'}</span></div></div><div class="panel-quote">${esc(T.flow)}</div></aside></div>
   </section>
 </div>`;
}
...
function seriesStatusLabel(x,en=false){return x.status==='done'?(en?'Completed':'مكتمل'):x.status==='paused'?(en?'Paused':'متوقف'):(en?'Active':'نشط')}
...
function islamicDayBoundaryMinutes(){
 const m=Number(state?.faith?.sunsetMinutes);
 return Number.isFinite(m)&&m>=0&&m<1440?m:1080;
}
function spiritualDate(date=new Date()){
 const d=new Date(date);
 const [hh,mm]=String(state?.faith?.prayerTimes?.maghrib||'18:00').split(':').map(Number);
 const fallback=Number.isFinite(hh)&&Number.isFinite(mm)?hh*60+mm:1080;
 const sunset=Number.isFinite(Number(state?.faith?.sunsetMinutes))?Number(state.faith.sunsetMinutes):fallback;
 const mins=d.getHours()*60+d.getMinutes();
 if(mins<sunset){d.setDate(d.getDate()-1);}
 d.setHours(12,0,0,0); return d;
}
function spiritualDateKey(date=new Date()){return localDateKey(spiritualDate(date));}
function islamicFromGregorian(g){
 const d=new Date(g); d.setHours(12,0,0,0);
...
const hijriMonthsEn=['','Muharram','Safar','Rabiʿ al-Awwal','Rabiʿ al-Thani','Jumada al-Awwal','Jumada al-Thani','Rajab','Shaʿban','Ramadan','Shawwal','Dhu al-Qaʿdah','Dhu al-Hijjah'];
function hijriLabel(h,en=false){return `${h.day} ${en?hijriMonthsEn[h.month]:hijriMonthsAr[h.month]} ${h.year}`;}
function spiritualSnapshot(){
 const sd=spiritualDate(), h=islamicFromGregorian(sd), key=localDateKey(sd);
 const prayers=['fajr','dhuhr','asr','maghrib','isha'];
 const labels={fajr:'الفجر',dhuhr:'الظهر',asr:'العصر',maghrib:'المغرب',isha:'العشاء'};
 const enLabels={fajr:'Fajr',dhuhr:'Dhuhr',asr:'Asr',maghrib:'Maghrib',isha:'Isha'};
 const log=state.faith?.prayerLog?.[key]||{};
 return {date:sd,key,h,prayers,labels,enLabels,log};
}
function fastingOpportunities(snapshot=spiritualSnapshot()){
 const {h}=snapshot, key=snapshot.key, log=state.faith?.fastingLog||{};
 const out=[];
 const civil=spiritualDate();
 const weekday=civil.getDay();
 if(weekday===1||weekday===4)out.push({id:`sunnah-${key}`,type:'weekly',title:'الاثنين / الخميس',enTitle:'Monday / Thursday',done:!!log[key],note:'فرصة أسبوعية'});
 if([13,14,15].includes(h.day))out.push({id:`white-${key}`,type:'white',title:`الأيام البيض — ${h.day}`,enTitle:`White days — ${h.day}`,done:!!log[key],note:'فرصة شهرية'});
 if(h.month===10)out.push({id:`shawwal-${key}`,type:'shawwal',title:'من شوال — ضمن الست',enTitle:'Shawwal — within the six',done:!!log[key],note:'سجل الأيام التي صمتها دون افتراض التتابع'});
 if(h.month===12 && h.day<=10 && h.day>=1)out.push({id:`dhulhijjah-${key}`,type:'dhulhijjah',title:`أول عشر ذي الحجة — ${h.day}`,enTitle:`First ten of Dhu al-Hijjah — ${h.day}`,done:!!log[key],note:'فرصة عامة'});
 if(h.month===12 && h.day===9)out.push({id:`arafah-${key}`,type:'arafah',title:'يوم عرفة — 9 ذو الحجة',enTitle:'ʿArafah — 9 Dhu al-Hijjah',done:!!log[key],note:'اليوم المميز في هذا الباب'});
 if(h.month===1 && (h.day===9||h.day===10||h.day===11))out.push({id:`ashura-${key}`,type:'ashura',title:`عاشوراء — ${h.day} محرم`,enTitle:`ʿAshura — ${h.day} Muharram`,done:!!log[key],note:'تتبع يوم 9 أو 11 بحسب اختيارك مع يوم 10'});
 return out;
}
function fastingSummary(){
 const log=state.faith?.fastingLog||{}; const snap=spiritualSnapshot();
 const shawwal=Object.entries(log).filter(([k,v])=>v&&k.includes('-')&&islamicFromGregorian(new Date(k+'T12:00:00')).month===10).length;
 return {count:Object.values(log).filter(Boolean).length,shawwal:Math.min(6,shawwal),today:!!log[snap.key]};
}
function istighfarToday(){
 const iso=spiritualDateKey(); const x=state.faith?.istighfar||{date:'',today:0,total:0};
 if(x.date!==iso){x.date=iso;x.today=0;state.faith.istighfar=x;save();}
 return x;
}
function recentStoryEvents(limit=5){return (state.history?.events||[]).slice().sort((a,b)=>String(b.at||'').localeCompare(String(a.at||''))).filter(e=>['task.complete','focus.complete','series.complete','series.session.complete'].includes(e.type)).slice(0,limit);}
function renderShari(){
 ...
 return `<div class="section-title"><div><span class="micro-label">01 / SANCTUARY ROOM</span><h2>🕌 ${en?'Islamic studies':'العلم الشرعي'}</h2><p>${en?'A protected room for learning. Series are data-driven and future tracks appear automatically.':'غرفة محمية للعلم. المسارات هنا بيانات عامة، وأي مسار شرعي جديد يظهر تلقائيًا.'}</p></div><span class="badge core">${en?'Protected':'أساسي'}</span></div><section class="shari-hero"><div><span class="micro-label">${en?'THE LIBRARY':'المكتبة'}</span><h2>${en?'Learn deliberately, not endlessly.':'اتعلم بقصد، مش بتكديس المحتوى.'}</h2><p>${en?'The page shows only the information needed to choose your next session. Details stay one interaction away.':'الصفحة تعرض فقط ما تحتاجه علشان تختار الجلسة الجاية؛ التفاصيل تفضل على بُعد ضغطة.'}</p></div><div class="shari-stats"><div><b>${active.length}</b><span>${en?'active':'نشط'}</span></div><div><b>${done.length}</b><span>${en?'completed':'مكتمل'}</span></div><div><b>${series.length}</b><span>${en?'total':'إجمالي'}</span></div></div></section>${group(en?'ACTIVE TRACKS':'المسارات النشطة',active,en?'Nothing active yet.':'مفيش مسارات نشطة دلوقتي.')}${group(en?'PAUSED / ARCHIVED':'المتوقف / المؤرشف',paused,en?'No paused tracks.':'مفيش مسارات متوقفة.')}${group(en?'COMPLETED / ACHIEVED':'مكتمل / منجز',done,en?'No completed tracks have been recorded yet.':'لسه مفيش مسارات مكتملة مسجلة.')}
 <div class="section-title"><div><span class="micro-label">02 / WEEKLY RHYTHM</span><h2>${en?'Weekly room rhythm':'إيقاع الغرفة الأسبوعي'}</h2><p>${en?'The schedule remains separate from the content library.':'الجدول الأسبوعي يفضل منفصل عن مكتبة المسارات.'}</p></div></div><div class="timeline">${rows}</div>
 <div class="section-title"><div><span class="micro-label">03 / TODAY</span><h2>${en?'Today’s protected work':'تنفيذ اليوم المحمي'}</h2><p>${en?'Mark only what you actually complete.':'علّم فقط اللي أنجزته فعلًا.'}</p></div></div><section class="section-box">${taskHTML(shariItems(day))}</section>
 <div class="section-title"><div><span class="micro-label">04 / SPIRITUAL CLOCK\</span><h2>${en?'Spiritual day & prayer rhythm':'اليوم الشرعي وإيقاع الصلاة'}</h2><p>${en?'A quiet in-app rhythm. Prayer times are editable reminders, not a fatwa or a prayer authority.':'تذكير هادئ داخل التطبيق. المواعيد قابلة للتعديل وليست مرجعًا شرعيًا للصلاة.'}</p></div></div>
 <section class="faith-clock"><div class="faith-date"><span class="micro-label">${en?'ISLAMIC DATE':'التاريخ الهجري'}</span><b>${esc(hijriLabel(spiritualSnapshot().h,en))}</b><small>${en?'Calculated calendar · spiritual day begins at your Maghrib setting.':'تقويم حسابي · اليوم الشرعي يبدأ وفق وقت المغرب الذي تضبطه.'}</small></div><div class="prayer-rail">${['fajr','dhuhr','asr','maghrib','isha'].map(p=>{const done=!!spiritualSnapshot().log[p];const label=en?spiritualSnapshot().enLabels[p]:spiritualSnapshot().labels[p];return `<button class="prayer-node ${done?'done':''}" onclick="togglePrayer('${p}')"><span>${esc(label)}</span><b>${esc(state.faith.prayerTimes[p])}</b><i>${done?'✓':'·'}</i></button>`}).join('')}</div></section>
 <section class="grid grid-2 faith-tools"><section class="section-box"><div class="section-title"><div><span class="micro-label">05 / FASTING FIELD</span><h3>${en?'Sunnah fasting':'صيام السنن'}</h3><p>${en?'Track opportunities without turning worship into a leaderboard.':'تتبع الفرص من غير ما نحول العبادة لسباق أرقام.'}</p></div><span class="badge">${fastingSummary().count}</span></div><div class="fasting-list">${fastingOpportunities().length?fastingOpportunities().map(f=>`<button class="fasting-row ${f.done?'done':''}" onclick="toggleFasting('${f.id}')"><span>${esc(en?f.enTitle:f.title)}</span><small>${esc(en?f.note:(f.note||''))}</small><i>${f.done?'✓':'○'}</i></button>`).join(''):`<div class="note">${en?'No dated opportunity is active right now.':'مفيش فرصة محددة داخلة النهارده حسب التاريخ الحسابي.'}</div>`}</div><div class="faith-note">${en?'Shawwal count':'عداد شوال'} <strong>${fastingSummary().shawwal}/6</strong> · ${en?'calendar dates are a planning aid; verify your adopted source.':'التواريخ هنا للمساعدة في التخطيط؛ راجع المصدر الذي تعتمد عليه.'}</div></section><section class="section-box"><div class="section-title"><div><span class="micro-label">06 / QUIET TALLY</span><h3>${en?'Istighfar counter':'عداد الاستغفار'}</h3><p>${en?'One calm tap. No streak pressure.':'ضغطة هادئة. من غير ضغط ولا لعبة تتابعات.'}</p></div><span class="badge">${istighfarToday().today}</span></div><button class="tally-orb" onclick="incrementIstighfar()" aria-label="${en?'Add one istighfar':'إضافة استغفار'}"><span>${istighfarToday().today}</span><small>${en?'today':'اليوم'}</small></button><div class="faith-note">${en?'All-time':'الإجمالي'} <strong>${istighfarToday().total}</strong></div></section></section>
 <section class="section-box privacy-faith"><div class="section-title"><div><span class="micro-label">07 / PRIVATE OBLIGATIONS</span><h3>${en?'Kaffarat & vows':'الكفارات والنذر'}</h3><p>${en?'Track a commitment without storing sensitive details. Fiqh-sensitive choices stay explicit.':'تابع الالتزام من غير تخزين تفاصيل حساسة. المسائل الخلافية تظل اختيارًا صريحًا منك.'}</p></div><button class="btn primary" onclick="addObligation()">＋ ${en?'Add':'إضافة'}</button></div><div class="obligation-list">${state.faith.obligations.length?state.faith.obligations.map(o=>`<div class="obligation-row"><div><b>${esc(en?(o.type==='kaffara'?'Yamin / kaffara':'Vow / nadhr'):(o.type==='kaffara'?'كفارة يمين':'نذر'))}</b><small>${o.completed?'✓ '+(en?'Completed':'مكتمل'):(en?'Open obligation':'التزام مفتوح')} · ${o.qty||1}</small></div><button class="tiny-action" onclick="toggleObligation('${o.id}')">${o.completed?(en?'Reopen':'إعادة فتح'):(en?'Mark done':'تم')}</button><button class="tiny-action" onclick="deleteObligation('${o.id}')">×</button></div>`).join(''):`<div class="note">${en?'Nothing tracked here.':'مفيش التزامات مسجلة هنا.'}</div>`}</div><div class="privacy-copy">${en?'Privacy by design: do not record confession-like details, names, locations, or sensitive circumstances. Use only a label and completion state.':'الخصوصية أولًا: ما تسجلش اعترافات أو أسماء أو أماكن أو تفاصيل حساسة. استخدم تسمية وحالة إنجاز فقط.'}</div></section>`;
}
...
function renderSystemBase(){
 ...
 <section class="section-box" style="margin-top:12px"><div class="section-title" style="margin:0 0 8px"><div><span class="micro-label">11 / SPIRITUAL CLOCK</span><h3 style="margin:0">${state.lang==='en'?'Prayer times & spiritual day':'مواقيت الصلاة واليوم الشرعي'}</h3><p>${state.lang==='en'?'Local, editable reminder times. The app does not treat them as a religious authority.':'مواعيد محلية قابلة للتعديل للتذكير فقط؛ التطبيق لا يتعامل معها كمرجع شرعي.'}</p></div><span class="badge">${state.faith.calendarSource==='tabular'?(state.lang==='en'?'Tabular':'حسابي'):(state.lang==='en'?'Manual':'يدوي')}</span></div><div class="prayer-settings">${[['fajr','الفجر'],['dhuhr','الظهر'],['asr','العصر'],['maghrib','المغرب'],['isha','العشاء']].map(([id,label])=>`<label class="time-field"><span>${state.lang==='en'?({fajr:'Fajr',dhuhr:'Dhuhr',asr:'Asr',maghrib:'Maghrib',isha:'Isha'}[id]):label}</span><input type="time" value="${esc(state.faith.prayerTimes[id])}" onchange="setPrayerTime('${id}',this.value)"></label>`).join('')}</div><div class="note">${state.lang==='en'?'The spiritual-day boundary uses Maghrib. The Hijri label is an arithmetic calendar aid; local moon-sighting or your adopted calendar may differ.':'حدّ اليوم الشرعي هنا هو المغرب. التاريخ الهجري للتخطيط بالحساب؛ وقد يختلف عن رؤية الهلال أو التقويم الذي تعتمد عليه.'}</div></section>\n <div class="section-box" style="margin-top:12px"><h3>🧩 ${state.lang==='en'?'Operating rules':'قواعد التشغيل'}</h3><div class="grid grid-2"><div>${(state.lang==='en'?['Sleep 6–8 hours.','During exams: regular Anki continues; new production reduces first.','If two days are lost in a row: no forced catch-up; review why.']:['النوم 6–8 ساعات.','امتحانات: Anki regular مستمر، والإنتاج الجديد يقل أولًا.','لو يومان ضاعا وراء بعض: لا تعويض قهري؛ راجع السبب.']).map(x=>`<p>${x}</p>`).join('')}</div><div>${(state.lang==='en'?['Islamic studies stay fixed.','Qur’an stays fixed but timing is flexible.','Marketing is the professional priority; McKinsey and Dose shrink first.']:['الشرعي ثابت.','القرآن ثابت لكن توقيته مرن.','Marketing هو الأولوية المهنية؛ McKinsey وDose أول من يتقلص.']).map(x=>`<p>${x}</p>`).join('')}</div></div></div>`;
}
...
(function(){
  ...
  function ensureState(){
    ...
    state.faith={sunsetMinutes:null,calendarSource:'tabular',prayerTimes:{fajr:'05:00',dhuhr:'12:00',asr:'15:30',maghrib:'18:00',isha:'19:30'},prayerLog:{},fastingLog:{},istighfar:{date:'',today:0,total:0},obligations:[],proofVault:[],...(state.faith||{})};
    state.faith.prayerTimes={fajr:'05:00',dhuhr:'12:00',asr:'15:30',maghrib:'18:00',isha:'19:30',...(state.faith.prayerTimes||{})};
    if(!Number.isFinite(Number(state.faith.sunsetMinutes))){const [mh,mm]=String(state.faith.prayerTimes.maghrib).split(':').map(Number);state.faith.sunsetMinutes=(Number.isFinite(mh)&&Number.isFinite(mm))?mh*60+mm:1080;}
    state.faith.prayerLog ||= {}; state.faith.fastingLog ||= {}; state.faith.obligations ||= []; state.faith.proofVault ||= []; state.faith.istighfar={date:'',today:0,total:0,...(state.faith.istighfar||{})};
    try{
  ...
  window.setPrayerTime=(id,value)=>{if(!['fajr','dhuhr','asr','maghrib','isha'].includes(id)||!/^[0-2]\d:[0-5]\d$/.test(value))return;state.faith.prayerTimes[id]=value;if(id==='maghrib')state.faith.sunsetMinutes=Number(value.slice(0,2))*60+Number(value.slice(3));save();rerender()};
  window.togglePrayer=p=>{const idMap={fajr:'pr_f',dhuhr:'pr_d',asr:'pr_a',maghrib:'pr_m',isha:'pr_i'},id=idMap[p];if(!id)return;toggleToday(id);const snap=spiritualSnapshot();snap.log[p]=!!state.today[id];state.faith.prayerLog[snap.key]=snap.log;save();rerender()};
  window.toggleFasting=id=>{const key=spiritualDateKey();state.faith.fastingLog[key]=!state.faith.fastingLog[key];recordEvent(state.faith.fastingLog[key]?'fasting.mark':'fasting.unmark',{id,date:key});save();rerender()};
  window.incrementIstighfar=()=>{const x=istighfarToday();x.today++;x.total++;state.faith.istighfar=x;recordEvent('istighfar.count',{amount:1,date:x.date});save();rerender()};
  window.addObligation=()=>{const en=state.lang==='en';openModal(`<div class="mihrab-modal-head"><b>${en?'Private obligation':'التزام خاص'}</b><button class="mihrab-close" onclick="closeMihrabModal()">×</button></div><div class="modal-body"><label class="field-lite"><span>${en?'Type':'النوع'}</span><select id="obType"><option value="kaffara">${en?'Yamin / Kaffara':'كفارة يمين'}</option><option value="nadhr">${en?'Vow / Nadhr':'نذر'}</option></select></label><label class="field-lite"><span>${en?'Quantity / count':'العدد'}</span><input id="obQty" type="number" min="1" value="1"></label><div class="note">${en?'No sensitive circumstances are stored here. This is a personal tracking tool; verify fiqh details with your trusted source.':'لن تُحفظ هنا تفاصيل حساسة. هذه أداة متابعة شخصية؛ راجع الحكم الفقهي والمصدر الذي تثق به.'}</div><div class="modal-actions"><button class="btn" onclick="closeMihrabModal()">${en?'Cancel':'إلغاء'}</button><button class="btn primary" onclick="saveObligation()">${en?'Save':'حفظ'}</button></div></div>`)};
  window.saveObligation=()=>{const type=$('#obType')?.value==='nadhr'?'nadhr':'kaffara';const qty=Math.max(1,Number($('#obQty')?.value)||1);state.faith.obligations.push({id:'obl_'+Date.now().toString(36),type,qty,completed:false,createdAt:new Date().toISOString()});recordEvent('obligation.create',{type,qty});save();closeModal();rerender()};
  window.toggleObligation=id=>{const o=state.faith.obligations.find(x=>x.id===id);if(!o)return;o.completed=!o.completed;recordEvent(o.completed?'obligation.complete':'obligation.reopen',{id,type:o.type});save();rerender()};
  window.deleteObligation=id=>{state.faith.obligations=state.faith.obligations.filter(x=>x.id!==id);save();rerender()};
  ...
  function systemFaithExtras(){
    const en=state.lang==='en', recent=recentStoryEvents(6), pv=state.faith?.proofVault||[];
    const recentHtml=recent.length?recent.map(e=>{const label=e.type==='task.complete'?(en?'Task completed':'تم إنجاز مهمة'):e.type==='focus.complete'?(en?'Focus session':'جلسة تركيز'):e.type==='series.complete'?(en?'Series completed':'اكتمل مسار'):e.type==='series.session.complete'?(en?'Study session':'جلسة دراسة'):'Milestone';const title=e.payload?.title||e.payload?.taskId||label;return `<div class=\"story-row\"><span class=\"story-dot\"></span><div><b>${esc(String(title))}</b><small>${esc(label)} · ${e.at?new Date(e.at).toLocaleDateString(en?'en':'ar-EG'):''}</small></div></div>`}).join(''):`<div class=\"note\">${en?'Your recent story will appear from real recorded work.':'قصتك الأخيرة هنا تُبنى من إنجازاتك المسجلة فعليًا.'}</div>`;
    const pvHtml=pv.length?pv.slice().reverse().slice(0,8).map(x=>`<div class=\"proof-row\"><div><b>${esc(x.title)}</b><small>${esc(x.kind||'proof')} · ${x.date?new Date(x.date).toLocaleDateString(en?'en':'ar-EG'):''}</small></div>${x.url?`<a class=\"tiny-action\" href=\"${esc(x.url)}\" target=\"_blank\" rel=\"noopener\">${en?'Open':'فتح'} ↗</a>`:''}</div>`).join(''):`<div class=\"note\">${en?'Keep proof lightweight: one title, optional link, no sensitive payloads.':'خلي الإثبات خفيفًا: عنوان، ورابط اختياري، من غير بيانات حساسة.'}</div>`;
    return `<section class=\"grid grid-2\"><section class=\"section-box\"><div class=\"section-title\"><div><span class=\"micro-label\">09 / RECENT STORY</span><h3>${en?'What you have actually done':'ما أنجزته فعلًا'}</h3><p>${en?'A quiet narrative from recorded events — not invented history.':'حكاية هادئة مبنية على الأحداث المسجلة — مش تاريخ متخيل.'}</p></div></div><div class=\"story-list\">${recentHtml}</div></section><section class=\"section-box\"><div class=\"section-title\"><div><span class=\"micro-label\">10 / PROOF VAULT</span><h3>${en?'Keep evidence of work':'خزنة الإثبات'}</h3><p>${en?'Portfolio-proof for meaningful work, kept local-first.':'توثيق خفيف لشغلك المهم، محلي أولًا.'}</p></div><button class=\"btn primary\" onclick=\"addProof()\">＋ ${en?'Add proof':'إضافة إثبات'}</button></div><div class=\"proof-list\">${pvHtml}</div></section></section>`;
  }
  window.addProof=()=>{const en=state.lang==='en';openModal(`<div class="mihrab-modal-head"><b>${en?'Add proof':'إضافة إثبات'}</b><button class="mihrab-close" onclick="closeMihrabModal()">×</button></div><div class="modal-body"><label class="field-lite"><span>${en?'Title':'العنوان'}</span><input id="proofTitle" placeholder="${en?'e.g. Portfolio case study screenshot':'مثال: Screenshot من مشروع البورتفوليو'}"></label><label class="field-lite"><span>${en?'Kind':'النوع'}</span><input id="proofKind" placeholder="${en?'Portfolio / certificate / milestone':'بورتفوليو / شهادة / محطة'}"></label><label class="field-lite"><span>${en?'Optional URL':'رابط اختياري'}</span><input id="proofUrl" type="url" placeholder="https://..."></label><div class="note">${en?'Avoid storing passwords, private client data, or sensitive personal information.':'ما تخزنش كلمات مرور أو بيانات عملاء خاصة أو معلومات شخصية حساسة.'}</div><div class="modal-actions"><button class="btn" onclick="closeMihrabModal()">${en?'Cancel':'إلغاء'}</button><button class="btn primary" onclick="saveProof()">${en?'Save':'حفظ'}</button></div></div>`)};
  window.saveProof=()=>{const title=$('#proofTitle')?.value.trim();if(!title)return;state.faith.proofVault.push({id:'proof_'+Date.now().toString(36),title,kind:$('#proofKind')?.value.trim()||'proof',url:$('#proofUrl')?.value.trim()||'',date:new Date().toISOString()});recordEvent('proof.add',{title});save();closeModal();rerender()};
  ...
  function systemExtras(){
    ...
    const yearCompare=`<section class="section-box insight-compare"><div class="section-title"><div><span class="micro-label">YEAR / FIELD</span><h3>${en?'Strongest & weakest month':'أقوى وأضعف شهر'}</h3><p>${en?'Only months with tracked plan data appear.':'تظهر فقط الشهور التي لديها بيانات خطة مسجلة.'}</p></div></div><div class="compare-grid"><div><small>${en?'Strongest':'الأقوى'}</small><b>${monthName(year.strongestMonth)}</b><span>${year.strongestMonth?year.strongestMonth.rate+'%':'—'}</span></div><div><small>${en?'Weakest':'الأضعف'}</small><b>${monthName(year.weakestMonth)}</b><span>${year.weakestMonth?year.weakestMonth.rate+'%':'—'}</span></div><div><small>${en?'Longest streak':'أطول تتابع'}</small><b>${year.bestStreak}</b><span>${en?'days':'أيام'}</span></div></div></section>`;
    const analytics=`<section class="section-box analytics-panel"><div class="section-title"><div><span class="micro-label">05 / HISTORY FIELD</span><h2>${en?'Consistency & reality':'الثبات والواقع'}</h2><p>${en?'Recorded history only. Unknown periods stay unknown.':'تاريخ مسجل فقط. الفترات غير المعروفة تظل غير معروفة.'}</p></div><span class="badge">Local-first</span></div><div class="metrics-field">${periodCard('WEEK',week,active(week))}${periodCard('MONTH',month,active(month))}${periodCard('QUARTER',quarter,active(quarter))}${periodCard('YEAR',year,active(year))}${periodCard('ALL-TIME',all,all.planned?`${all.activeDays} ${en?'active days':'أيام نشطة'}`:(en?'History growing':'التاريخ بيتبني'))}</div><div class="long-signal" style="margin-top:10px"><div><span class="micro-label">REALITY / PLAN</span><h3>${en?'Nothing gets carried forward as punishment.':'مفيش ترحيل عقابي للي فات.'}</h3></div><div class="signal-copy"><strong>${all.completed}</strong><span>${en?'completed tasks':'مهمات مكتملة'}</span><em>·</em><strong>${all.sessions}</strong><span>${en?'focus sessions':'جلسات تركيز'}</span><em>·</em><strong>${all.currentStreak}</strong><span>${en?'current streak':'التتابع الحالي'}</span></div></div></section>`;
    return analytics+yearCompare+`<section class="grid grid-2 history-grid"><section class="section-box"><div class="section-title"><div><span class="micro-label">06 / RECENT WINS</span><h3>${en?'Recently completed':'تم إنجازه مؤخرًا'}</h3><p>${en?'Dated achievements only; older unknown dates stay unknown.':'الإنجازات المؤرخة فقط؛ التواريخ القديمة غير المعروفة لا يتم اختلاقها.'}</p></div></div><div class="history-list">${recentHtml}</div></section><section class="section-box"><div class="section-title"><div><span class="micro-label">07 / MILESTONES</span><h3>${en?'Achievement timeline':'خط الزمن للإنجاز'}</h3><p>${en?'25 / 50 / 75 / 100% — a record of what you actually built.':'25 / 50 / 75 / 100% — سجل لما بنيته فعلًا.'}</p></div></div><div class="history-list">${milestonesHtml}</div></section></section><section class="section-box lifecycle-card"><div class="section-title"><div><span class="micro-label">08 / LIBRARY</span><h2>${en?'Long-run system':'النظام على المدى الطويل'}</h2><p>${en?'The plan stays yours; content can evolve without editing code.':'الخطة ملكك، والمحتوى يتغير من غير لمس الكود.'}</p></div><button class="btn primary" onclick="addContent()">＋ ${en?'Add track':'إضافة مسار'}</button></div><div class="lib-tabs">${tabsHtml}</div><div class="library-list">${list}</div></section><div class="grid grid-2 lifecycle-grid"><section class="section-box"><h3>${en?'Inbox':'صندوق الوارد'}</h3><p class="muted">${en?'Quick ideas land here first.':'أي فكرة سريعة تدخل هنا الأول.'}</p>${inbox.length?inbox.slice(0,8).map(x=>`<div class="inbox-item"><div class="grow"><b>${esc(x.text)}</b><small>${x.duration}m</small></div><button class="tiny-action" onclick="completeInbox('${x.id}')">${en?'Done':'تم'}</button><button class="tiny-action" onclick="deleteInbox('${x.id}')">×</button></div>`).join(''):`<div class="note">${en?'Inbox is clear.':'صندوق الوارد فاضي.'}</div>`}<button class="btn" onclick="openQuickCapture()">＋ ${en?'Quick capture':'إضافة سريعة'}</button></section><section class="section-box"><h3>${en?'Backup & restore':'النسخ الاحتياطي'}</h3><p class="muted">${en?'Keep a JSON backup before moving devices or making major changes.':'خُد نسخة JSON قبل نقل الجهاز أو أي تغيير كبير.'}</p><div class="modal-actions"><button class="btn primary" onclick="exportMihrab()">${en?'Export JSON':'تصدير JSON'}</button><button class="btn" onclick="openImport()">${en?'Import':'استيراد'}</button></div></section></div>`;
  }
  const renderSystem=()=>renderSystemBase()+systemFaithExtras()+systemExtras();
