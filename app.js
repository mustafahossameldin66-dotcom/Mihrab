...
function dailyMomentum(){
 ...
 state.dailyMomentum ||= {};
 if(state.dailyMomentum.date!==today){
  let seed=0; for(let i=0;i<today.length;i++) seed=(seed*31+today.charCodeAt(i))>>>0;
  state.dailyMomentum={date:today,index:seed%messages.length}; save();
 }
...
function renderHome(){
 ...
 const label=x=>x?esc(x.label):(en?'Nothing queued.':'مفيش حاجة متبقية.');
 const longStats=(()=>{const h=state.history?.events||[];const days=new Set(h.filter(e=>e.type==='task.complete'||e.type==='focus.complete').map(e=>e.at.slice(0,10)));return {activeDays:days.size,events:h.length};})();
 const T=en?{
  morning:'TODAY / MIHRAB',start:'Enter today',today:'Today',progress:'Daily trajectory',mode:'Operating mode',next:'NEXT',later:'LATER',now:'NOW',focus:'Enter Focus',remaining:'remaining',days:'active days',brief:'Daily brief',open:'Open',execute:'Today’s field',rules:'Fixed pillars',rule1:'Reviews before new cards',rule2:'Islamic studies + Qur’an stay protected',rule3:'Courses shrink first',insight:'Long-term signal',week:'This week',streak:'Current streak',completed:'Recently completed',quiet:'Quietly building, without turning life into a dashboard.'
 }:{
  morning:'مِحْرَاب / اليوم',start:'ادخل يومك',today:'اليوم',progress:'مسار اليوم',mode:'وضع التشغيل',next:'التالي',later:'لاحقًا',now:'الآن',focus:'ابدأ التركيز',remaining:'متبقي',days:'أيام نشطة',brief:'اللقطة اليومية',open:'افتح',execute:'ساحة التنفيذ اليوم',rules:'الأركان الثابتة',rule1:'المراجعات قبل الجديد',rule2:'الشرعي والقرآن يفضلوا ثابتين',rule3:'الكورسات تتقلص أولًا',insight:'الإشارة طويلة المدى',week:'هذا الأسبوع',streak:'التتابع الحالي',completed:'آخر إنجاز',quiet:'بناء هادئ ومستمر، من غير ما تتحول حياتك إلى لوحة إحصائيات.'};
 return `<div class="spatial-home">
   <div class="spatial-kicker"><span class="signal-dot"></span><span>${T.morning}</span><span class="spatial-date">${esc(en?translateText(day):day)} · ${esc(modeLabel)}</span></div>
   <section class="spatial-hero">
     <div class="hero-surface hero-surface-main">
       <div class="hero-halo halo-a"></div><div class="hero-halo halo-b"></div><div class="hero-grid"></div>
       <div class="hero-copy">
         <div class="eyebrow">${en?'A quiet operating system for a meaningful day.':'مساحة هادئة لتشغيل يوم له معنى.'}</div>
         <h1>${en?'What matters<br><span>now.</span>':'إيه اللي<br><span>مهم دلوقتي؟</span>'}</h1>
         <p>${esc(dailyMomentum())}</p>
         <div class="hero-rail"><span></span><i>${T.quiet}</i></div>
         <div class="hero-actions spatial-actions"><button class="btn primary" onclick="document.getElementById('todayTasks').scrollIntoView({behavior:'smooth'})">${T.start}</button><button class="btn ghost" onclick="startFocus()">${T.focus}</button></div>
       </div>
       <div class="hero-orbit" aria-hidden="true">
         <div class="orbit orbit-outer"></div><div class="orbit orbit-mid"></div><div class="orbit orbit-inner"></div>
         <div class="orbit-core"><span>${pr.p}%</span><small>${done} / ${total}</small></div>
         <div class="orbit-node node-a"></div><div class="orbit-node node-b"></div><div class="orbit-node node-c"></div>
       </div>
     </div>
     <aside class="hero-surface hero-surface-side">
       <div class="side-top"><span class="micro-label">${T.progress}</span><strong>${pr.p}%</strong></div>
       <div class="energy-track"><i style="width:${pr.p}%"></i></div>
       <div class="spatial-pulse"></div>
       <div class="side-block"><span>${T.mode}</span><b>${modeLabel}</b><div class="mode-row">${['كلية','بدون كلية','ديب وورك'].map(t=>`<button class="mode ${todayDayType()===t?'active':''}" onclick="setDayType('${t}')">${en?translateText(t):t}</button>`).join('')}</div></div>
       <div class="side-metrics"><div><small>${T.remaining}</small><b>${remaining}</b></div><div><small>${T.days}</small><b>${longStats.activeDays}</b></div></div>
     </aside>
   </section>
   <section class="now-field">
     <div class="field-head"><div><span class="micro-label">${T.brief}</span><h2>${T.execute}</h2></div><span class="field-status">${done}/${total}</span></div>
     <div class="triad">
       <article class="triad-card triad-now"><span class="triad-mark">01</span><div><small>${T.now}</small><h3>${label(now)}</h3><p>${now?`${now.duration} ${en?'min':'د'} · ${now.priority==='core'?(en?'essential':'أساسي'):now.priority==='important'?(en?'important':'مهم'):(en?'optional':'اختياري')}`:''}</p></div><button class="triad-action" onclick="${now?`startFocus('${esc(now.id)}')`:'navigate(\'system\')'}">↗</button></article>
       <article class="triad-card"><span class="triad-mark">02</span><div><small>${T.next}</small><h3>${label(next)}</h3><p>${next?`${next.duration} ${en?'min':'د'}`:''}</p></div><button class="triad-action" onclick="${next?`startFocus('${esc(next.id)}')`:'navigate(\'system\')'}">↗</button></article>
       <article class="triad-card"><span class="triad-mark">03</span><div><small>${T.later}</small><h3>${label(later)}</h3><p>${later?`${later.duration} ${en?'min':'د'}`:''}</p></div><button class="triad-action" onclick="${later?`startFocus('${esc(later.id)}')`:'navigate(\'system\')'}">↗</button></article>
     </div>
   </section>
   <section class="stat-line">
     <div><span>${T.today}</span><b>${done}/${total}</b><small>${T.progress}</small></div>
     <div><span>${en?'Marketing':'التسويق'}</span><b>${state.weekly.marketingHours||0}${en?'h':'س'}</b><small>${en?'weekly load':'جرعة أسبوعية'}</small></div>
     <div><span>${T.streak}</span><b>${currentStreak()}</b><small>${en?'days':'يوم'}</small></div>
     <div><span>${T.completed}</span><b>${(state.history?.events||[]).filter(e=>e.type==='task.complete').length}</b><small>${en?'recorded events':'أحداث مسجلة'}</small></div>
   </section>
   <section class="section-title spatial-section-title" id="todayTasks"><div><span class="micro-label">02 / EXECUTION FIELD</span><h2>${T.execute}</h2><p>${esc(T.quiet)}</p></div></section>
   <div class="bento spatial-bento"><section class="card tall task-chamber"><div class="chamber-header"><div><span class="micro-label">TODAY</span><h3>${en?'Daily duties':'فروض اليوم وبناء النفس'}</h3></div><span class="chamber-count">${remaining} ${T.remaining}</span></div>${taskHTML(items)}</section>
   <section class="card portal-card"><span class="micro-label">03 / PORTALS</span><h3>${en?'Three rooms. One world.':'ثلاث غرف، وعالم واحد.'}</h3><div class="portal-list"><button onclick="navigate('marketing')"><i>01</i><span>${en?'Marketing':'التسويق'}</span><b>↗</b></button><button onclick="navigate('shari')"><i>02</i><span>${en?'Islamic studies':'الشرعي'}</span><b>↗</b></button><button onclick="navigate('quran')"><i>03</i><span>${en?'Qur’an':'القرآن'}</span><b>↗</b></button></div></section>
   <section class="card principles-card"><span class="micro-label">04 / PILLARS</span><h3>${T.rules}</h3><div class="principle-list"><div><b>01</b><span>${T.rule1}</span></div><div><b>02</b><span>${T.rule2}</span></div><div><b>03</b><span>${T.rule3}</span></div></div></section></div>
   <section class="long-signal"><div><span class="micro-label">05 / LONG SIGNAL</span><h3>${T.insight}</h3></div><div class="signal-copy"><strong>${longStats.activeDays}</strong><span>${T.days}</span><em>·</em><strong>${currentStreak()}</strong><span>${T.streak}</span></div><button class="btn ghost" onclick="navigate('system')">${T.open} ↗</button></section>
 </div>`
}
...
function consistencyMetrics(period='week'){
 const now=effectiveDate(), start=new Date(now);
 const days=period==='week'?7:period==='month'?30:period==='quarter'?90:365;
 start.setDate(now.getDate()-(days-1));
 const startKey=localDateKey(start);
 const plans=state.history?.plans||{}; const events=state.history?.events||[];
 const latest=new Map(), sessionsByDay=new Map();
 for(const e of events){const day=e.day||e.at.slice(0,10);if(day<startKey)continue; if(e.type==='focus.complete'){sessionsByDay.set(day,(sessionsByDay.get(day)||0)+1);continue} if(!e.taskId||!['task.complete','task.uncomplete'].includes(e.type))continue; latest.set(day+'|'+e.taskId,e.type==='task.complete')}
 const completedByDay=new Map(); for(const [k,on] of latest){if(!on)continue;const i=k.indexOf('|'),day=k.slice(0,i);completedByDay.set(day,(completedByDay.get(day)||0)+1)}
 const activeDays=[...completedByDay.keys()].filter(d=>d>=startKey).length;
 const planned=new Set(), completed=new Set(); Object.entries(plans).forEach(([day,ids])=>{if(day<startKey)return;(ids||[]).forEach(id=>planned.add(day+'|'+id))});
 for(const [day,count] of completedByDay){for(let i=0;i<count;i++)completed.add(day+'|'+i)}
 const plannedCount=planned.size, completedCount=Math.min([...completedByDay.values()].reduce((a,b)=>a+b,0),plannedCount||Infinity);
 const sessions=[...sessionsByDay.values()].reduce((a,b)=>a+b,0);
 return {activeDays,planned:plannedCount,completed:completedCount,sessions,consistency:plannedCount?Math.round(completedCount/plannedCount*100):0};
}
...
(function(){
  ...
  function setLibraryStatus(id,status){const x=state.library.find(i=>i.id===id);if(!x)return; const prev=x.status; if(status==='done' && prev!=='done'){ const completedAt=new Date().toISOString(); x.status='done'; x.completedAt=completedAt; recordEvent('series.complete',{seriesId:id,title:x.title,titleEn:x.titleEn||'',category:x.category,completedAt}); if(!state.history.series[id]) state.history.series[id]={seriesId:id,title:x.title,titleEn:x.titleEn||'',category:x.category,importance:x.core?'core':x.important?'important':'optional',totalSessions:x.sessions||0,completedSessions:x.completedSessions||0,startDate:x.createdAt||null,completionDate:completedAt}; } else { x.status=status; } save();closeModal();rerender()}
  ...
  function systemExtras(){
    ...
    const week=consistencyMetrics('week'), month=consistencyMetrics('month'), year=consistencyMetrics('year');
    const analytics=`<section class="section-box analytics-panel"><div class="section-title"><div><span class="micro-label">05 / HISTORY FIELD</span><h2>${en?'Consistency & reality':'الثبات والواقع'}</h2><p>${en?'Only recorded events are counted. Unknown history stays unknown.':'لا يتحسب إلا ما تم تسجيله فعلًا. التاريخ غير المعروف يظل غير معروف.'}</p></div><span class="badge">Local-first</span></div><div class="grid grid-3"><div class="timeline-card"><span class="micro-label">WEEK</span><b style="font-size:22px">${week.consistency}%</b><p class="muted tiny">${week.activeDays}/${7} ${en?'active days':'أيام نشطة'} · ${week.completed} ${en?'completed':'منجز'}</p></div><div class="timeline-card"><span class="micro-label">MONTH</span><b style="font-size:22px">${month.consistency}%</b><p class="muted tiny">${month.activeDays}/${30} ${en?'active days':'أيام نشطة'} · ${month.sessions} ${en?'focus sessions':'جلسات تركيز'}</p></div><div class="timeline-card"><span class="micro-label">YEAR</span><b style="font-size:22px">${year.consistency}%</b><p class="muted tiny">${year.activeDays}/${365} ${en?'active days':'أيام نشطة'} · ${(state.history?.series&&Object.keys(state.history.series).length)||0} ${en?'completed series':'مسارات مكتملة'}</p></div></div><div class="long-signal" style="margin-top:9px"><div><span class="micro-label">REALITY / PLAN</span><h3>${en?'No punishment for missed work.':'مفيش عقوبة على اللي فات.'}</h3></div><div class="signal-copy"><strong>${week.completed}</strong><span>${en?'recorded completions':'إنجازات مسجلة'}</span><em>·</em><strong>${currentStreak()}</strong><span>${en?'day streak':'يوم تتابع'}</span></div></div></section>`;
    return analytics+`<section class="section-box lifecycle-card"><div class="section-title"><div><h2>${en?'Long-run system':'النظام على المدى الطويل'}</h2><p>${en?'The plan stays yours; content can evolve without editing code.':'الخطة ملكك، والمحتوى يتغير من غير لمس الكود.'}</p></div><button class="btn primary" onclick="addContent()">＋ ${en?'Add track':'إضافة مسار'}</button></div><div class="lib-tabs">${tabsHtml}</div><div class="library-list">${list}</div></section><div class="grid grid-2 lifecycle-grid"><section class="section-box"><h3>${en?'Inbox':'صندوق الوارد'}</h3><p class="muted">${en?'Quick ideas land here first.':'أي فكرة سريعة تدخل هنا الأول.'}</p>${inbox.length?inbox.slice(0,8).map(x=>`<div class="inbox-item"><div class="grow"><b>${esc(x.text)}</b><small>${x.duration}m</small></div><button class="tiny-action" onclick="completeInbox('${x.id}')">${en?'Done':'تم'}</button><button class="tiny-action" onclick="deleteInbox('${x.id}')">×</button></div>`).join(''):`<div class="note">${en?'Inbox is clear.':'صندوق الوارد فاضي.'}</div>`}<button class="btn" onclick="openQuickCapture()">＋ ${en?'Quick capture':'إضافة سريعة'}</button></section><section class="section-box"><h3>${en?'Backup & restore':'النسخ الاحتياطي'}</h3><p class="muted">${en?'Keep a JSON backup before moving devices or making major changes.':'خُد نسخة JSON قبل نقل الجهاز أو أي تغيير كبير.'}</p><div class="modal-actions"><button class="btn primary" onclick="exportMihrab()">${en?'Export JSON':'تصدير JSON'}</button><button class="btn" onclick="openImport()">${en?'Import':'استيراد'}</button></div></section></div>`;
  }
