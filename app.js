...
function completedTaskDates(){
 ...
 for(const e of state.history?.events||[]){if(!['task.complete','task.uncomplete','focus.complete'].includes(e.type)||!e.taskId)continue; const day=e.at.slice(0,10); latest.set(e.taskId+'|'+day,e.type!=='task.uncomplete');}
 const dates=new Set(); for(const [key,on] of latest) if(on) dates.add(key.slice(key.lastIndexOf('|')+1)); return dates;
}
function currentStreak(){
 const dates=completedTaskDates(); if(!dates.size)return 0; let d=new Date(); let key=d.toISOString().slice(0,10); let n=0; while(dates.has(key)){n++;d.setDate(d.getDate()-1);key=d.toISOString().slice(0,10)} return n;
}
function consistencyMetrics(period='week'){
 ...
 else if(period==='year') start.setDate(now.getDate()-364);
 const key=e=>e.at.slice(0,10), startKey=start.toISOString().slice(0,10);
 const events=state.history?.events||[], relevant=events.filter(e=>key(e)>=startKey && ['task.complete','focus.complete'].includes(e.type));
 const dates=new Set(relevant.map(key));
 const completed=relevant.filter(e=>e.type==='task.complete').length;
 const sessions=relevant.filter(e=>e.type==='focus.complete').length;
 return {activeDays:dates.size,completed,sessions,consistency:period==='week'?Math.round(dates.size/7*100):period==='month'?Math.round(dates.size/30*100):period==='quarter'?Math.round(dates.size/90*100):Math.round(dates.size/365*100)};
}
function recordEvent(type,payload={}){
 state.history ||= {events:[],series:{},milestones:[]}; state.history.events ||= [];
 state.history.events.push({id:'evt_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,7),type,at:new Date().toISOString(),...payload});
...
(function(){
  ...
  function ensureState(){
    ...
    state.history={events:[],series:{},milestones:[],...(state.history||{})}; state.history.events ||= []; state.history.series ||= {}; state.history.milestones ||= [];
  ...
  function setLibraryStatus(id,status){const x=state.library.find(i=>i.id===id);if(!x)return; const prev=x.status; x.status=status; if(status==='done' && prev!=='done'){ x.completedAt=x.completedAt||new Date().toISOString(); recordEvent('series.complete',{seriesId:id,title:x.title,titleEn:x.titleEn||'',category:x.category,completedAt:x.completedAt}); if(!state.history.series[id]) state.history.series[id]={seriesId:id,title:x.title,titleEn:x.titleEn||'',category:x.category,importance:x.core?'core':x.important?'important':'optional',totalSessions:x.sessions||0,completedSessions:x.completedSessions||0,startDate:x.createdAt||null,completionDate:x.completedAt||null}; } save();closeModal();rerender()}
