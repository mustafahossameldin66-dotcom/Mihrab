...
let state={theme:'mono',lang:'ar',view:'home',dayType:'كلية',todayDate:'',today:{},plan:{},weekly:{marketingHours:0,mckinsey:false,dose:false,review:false,rating:''},weekDayTypes:{},quranFrameOpen:false,mode:'normal',modeDate:''};
function esc(s){return String(s).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}
function save(){localStorage.setItem(KEY,JSON.stringify(state))}
function load(){try{const raw=localStorage.getItem(KEY);if(raw){const x=JSON.parse(raw);state={...state,...x,lang:x.lang||state.lang||'ar',today:x.today||{},plan:x.plan||{},weekly:{...state.weekly,...(x.weekly||{})},weekDayTypes:x.weekDayTypes||{},quranFrameOpen:!!x.quranFrameOpen,mode:x.mode||'normal',modeDate:x.modeDate||''};}else{const old=localStorage.getItem(OLD_KEY);if(old){const x=JSON.parse(old);state.today=x.today||{};state.plan=x.plan||{};state.weekly={...state.weekly,...(x.weekly||{})};}}}catch(e){} resetDay(); applyTheme();}
...
/* ================================================================
   Mihrab Performance Core — targeted updates / no rerender on check
   ================================================================ */
(function MihrabTargetedUpdates(){
  const sleep=(ms=0)=>new Promise(r=>setTimeout(r,ms));
  function updateRing(){
    const items=dayTasks(todayName());
  ...
  function markRow(id,done){
    ...
    const cb=row.querySelector('input[type="checkbox"]'); if(cb) cb.checked=done;
  }
  ...
  window.togglePlan=function(id){
    ...
    save();
    const els=document.querySelectorAll(`[onchange*="togglePlan('${CSS.escape(id)}')"]`);
    els.forEach(input=>{input.checked=!!state.plan[id]; input.closest('.mtask,.lecture')?.classList.toggle('done',!!state.plan[id]);});
    document.querySelectorAll('.week-card').forEach(card=>{
      const inputs=[...card.querySelectorAll('input[onchange*="togglePlan"]')];
      if(!inputs.length)return;
      const ids=inputs.map(i=>{const m=i.getAttribute('onchange')?.match(/togglePlan\('([^']+)/);return m?.[1]}).filter(Boolean);
      const pr=pct(ids,state.plan); const badge=card.querySelector('summary .badge'); if(badge)badge.textContent=`${pr.p}% · ${pr.n}/${pr.total}`;
      const pb=card.querySelector('.progress>i'); if(pb)pb.style.width=pr.p+'%';
      const pv=card.querySelector('.progress-head b'); if(pv)pv.textContent=pr.p+'%';
    });
  };
  window.setWeeklyRating=function(v){state.weekly.rating=v;state.weekly.review=true;save();document.querySelectorAll('.review-choice').forEach(el=>{const r=el.querySelector('input[type="radio"]');el.classList.toggle('selected',r?.value===v);if(r)r.checked=r.value===v});const badge=document.querySelector('#view-system .section-box .badge');if(badge)badge.textContent=state.lang==='en'?'Saved':'محفوظ';};
  // patch course toggles that previously used inline renderAll
  document.addEventListener('change',e=>{
    const el=e.target;
    if(!(el instanceof HTMLInputElement) || el.type!=='checkbox') return;
    if(!el.getAttribute('onchange')?.includes('renderAll()')) return;
    const m=el.getAttribute('onchange').match(/state\\.weekly\\.(\\w+)=this\\.checked/); if(!m)return;
    state.weekly[m[1]]=el.checked; save(); el.closest('.task')?.classList.toggle('done',el.checked);
...
(function MihrabLifelong(){
  const ensure=()=>{
    ...
    state.settings ||= {ambient:true,density:'comfortable',timeFormat:'24h',autoBackup:true,lowPower:false};
  ...
  const commands=()=>{
    ...
    return [
      ...
      ['focus',en?'Start Focus Mode':'ابدأ وضع التركيز','focus'],['capture',en?'Quick Capture':'إضافة سريعة','capture'],['15',en?'I have 15 minutes':'عندي 15 دقيقة','smart15'],['30',en?'I have 30 minutes':'عندي 30 دقيقة','smart30'],['energy',en?'No Energy':'مفيش طاقة','low'],['exam',en?'Exam mode':'وضع الامتحانات','exam'],['survival',en?'Survival mode':'وضع النجاة','survival'],['export',en?'Export backup':'تصدير نسخة احتياطية','export']
    ];
  };
  ...
  window.runCommand=(action)=>{closeMihrabModal();if(action==='focus')return startFocus();if(action==='capture')return openQuickCapture();if(action==='smart15')return smartTime(15);if(action==='smart30')return smartTime(30);if(action==='low')return setMode('busy');if(action==='exam')return setMode('exam');if(action==='survival')return setMode('survival');if(action==='export')return exportMihrab();if(action.startsWith('go-'))return navigate(action.slice(3));};
  ...
  window.setMode=mode=>{state.mode=mode;state.modeDate=keyDate();save();renderAll()};
  function activeMode(){return state.modeDate===keyDate()?state.mode:(dayTypeToMode[todayDayType()]||'normal')}
  function modeAllows(id){const m=activeMode();const p=priorityOf(id);if(m==='rest')return false;if(m==='survival')return ['pr_f','pr_d','pr_a','pr_m','pr_i','azkar','quran','rafiq','aw','zad'].includes(id);if(m==='exam')return p==='core'||id==='marketing';if(m==='busy')return p!=='optional';return true}
  ...
  window.startFocus=()=>{
    ...
    ensureModals();
    const fm=document.getElementById('focusMode');fm.classList.add('open');
    const seconds=(x.duration||25)*60; let end=Date.now()+seconds*1000; let raf=0,completed=false;
    ...
    const paint=()=>{
      ...
      const timerEl=shell.querySelector('.focus-timer'); if(timerEl) timerEl.textContent=`${m}:${sec}`;
      if(left<=0){
        if(!completed){completed=true;state.metrics.focusMinutes+=(x.duration||25);state.metrics.sessions+=1;save();}
        cancelAnimationFrame(raf);return;
      }
      raf=requestAnimationFrame(paint);
    };
    shell.innerHTML=`<div class="focus-kicker">${en?'MIHRAB FOCUS':'تركيز مِحْرَاب'}</div><h2>${esc(en&&TASK_EN[x.id]?TASK_EN[x.id]:x.label)}</h2><div class="focus-timer"></div><div class="focus-actions"><button class="btn primary" onclick="finishFocus('${esc(x.id)}');closeFocus()">${en?'Mark done':'تم'}</button><button class="btn" onclick="extendFocus(5)">+5m</button><button class="btn" onclick="closeFocus()">${en?'Exit':'خروج'}</button></div>`;
    paint();
    window.__focusTimer=raf; window.__focusEnd=()=>end;
    window.__focusPaint=paint;
  };
  window.extendFocus=min=>{if(window.__focusEnd){let end=window.__focusEnd()+min*60000;window.__focusEnd=()=>end;cancelAnimationFrame(window.__focusTimer||0);window.__focusPaint?.();}};
  window.closeFocus=()=>{document.getElementById('focusMode')?.classList.remove('open');cancelAnimationFrame(window.__focusTimer||0);window.__focusTimer=0};
  ...
  window.archiveContent=id=>{const x=state.library.find(i=>i.id===id);if(!x)return; if(!confirm(state.lang==='en'?`Archive “${x.titleEn||x.title}”? It will leave the active plan and remain in history.`:`أرشفة «${x.title}»؟ هتخرج من الخطة النشطة وتفضل محفوظة في السجل.`))return;x.status='paused';save();renderAll()};
  window.activateContent=id=>{const x=state.library.find(i=>i.id===id);if(!x)return;x.status='active';save();renderAll()};
  window.replaceContent=id=>{const x=state.library.find(i=>i.id===id);if(!x)return;const en=state.lang==='en';openModal(`<div class="mihrab-modal-head"><b>${en?'Replace track':'استبدال المسار'}</b><button class="mihrab-close" onclick="closeMihrabModal()">×</button></div><div class="modal-body"><p class="muted">${en?'Archive this track and create a new one with the same schedule and category.':'أرشف المسار الحالي وأضف مسارًا جديدًا بنفس الأيام والقسم.'}</p><div class="form-grid"><div class="field-lite"><label>${en?'New title':'الاسم الجديد'}</label><input id="replaceTitle"></div><div class="field-lite"><label>${en?'English title':'الاسم بالإنجليزي'}</label><input id="replaceTitleEn"></div></div><div class="modal-actions"><button class="btn" onclick="closeMihrabModal()">${en?'Cancel':'إلغاء'}</button><button class="btn primary" onclick="saveReplacement('${id}')">${en?'Replace':'استبدال'}</button></div></div>`)};
  window.saveReplacement=id=>{const old=state.library.find(i=>i.id===id);if(!old)return;const title=document.getElementById('replaceTitle')?.value.trim();if(!title)return;old.status='paused';state.library.unshift({id:'lib_'+Date.now().toString(36),title,titleEn:document.getElementById('replaceTitleEn')?.value.trim()||'',category:old.category,group:old.group||'',days:[...(old.days||[])],duration:old.duration||30,core:!!old.core,important:!!old.important,status:'active',sessions:0,completedSessions:0,createdAt:new Date().toISOString(),replaces:id});save();closeMihrabModal();renderAll()};
  window.editContent=id=>{const x=state.library.find(i=>i.id===id);if(!x)return;const en=state.lang==='en';openModal(`<div class="mihrab-modal-head"><b>${en?'Edit track':'تعديل المسار'}</b><button class="mihrab-close" onclick="closeMihrabModal()">×</button></div><div class="modal-body"><div class="form-grid"><div class="field-lite"><label>${en?'Arabic title':'الاسم'}</label><input id="editTitle" value="${esc(x.title||'')}"></div><div class="field-lite"><label>${en?'English title':'الاسم بالإنجليزي'}</label><input id="editTitleEn" value="${esc(x.titleEn||'')}"></div><div class="field-lite"><label>${en?'Minutes':'الدقائق'}</label><input id="editDur" type="number" min="1" value="${x.duration||30}"></div><div class="field-lite"><label>${en?'Status':'الحالة'}</label><select id="editStatus"><option value="active" ${x.status==='active'?'selected':''}>${en?'Active':'نشط'}</option><option value="paused" ${x.status==='paused'?'selected':''}>${en?'Paused':'متوقف'}</option><option value="done" ${x.status==='done'?'selected':''}>${en?'Completed':'مكتمل'}</option></select></div></div><div class="modal-actions"><button class="btn" onclick="closeMihrabModal()">${en?'Cancel':'إلغاء'}</button><button class="btn primary" onclick="saveEditedContent('${id}')">${en?'Save':'حفظ'}</button></div></div>`)};
  ...
  window.addEventListener('offline',()=>{document.body.dataset.online='false';document.getElementById('mihrab-online-dot')?.setAttribute('title','Offline')}); document.body.dataset.online=navigator.onLine?'true':'false'; document.body.dataset.lowPower=state.settings?.lowPower?'true':'false';
...
/* =====================================================================
   Mihrab Final i18n layer — render English directly, no TreeWalker
   ===================================================================== */
(function MihrabDirectI18n(){
  const en=()=>state.lang==='en';
  const PHASE_EN={
  ...
  const replacements=[
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*مشاهدة فيديو عن /i,(m,n,t)=>`Day ${n} · ${t}: Watch a video about `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*فهم /i,(m,n,t)=>`Day ${n} · ${t}: Understand `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*قراءة عن /i,(m,n,t)=>`Day ${n} · ${t}: Read about `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*فيديو تطبيقي /i,(m,n,t)=>`Day ${n} · ${t}: Practical walkthrough — `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*تصفح /i,(m,n,t)=>`Day ${n} · ${t}: Explore `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*جولة في /i,(m,n,t)=>`Day ${n} · ${t}: Tour `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*صياغة /i,(m,n,t)=>`Day ${n} · ${t}: Create `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*التدريب على /i,(m,n,t)=>`Day ${n} · ${t}: Practice `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*دراسة /i,(m,n,t)=>`Day ${n} · ${t}: Study `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*استخدام /i,(m,n,t)=>`Day ${n} · ${t}: Use `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*رسم /i,(m,n,t)=>`Day ${n} · ${t}: Map `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*حفظ /i,(m,n,t)=>`Day ${n} · ${t}: Memorize `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*إنشاء /i,(m,n,t)=>`Day ${n} · ${t}: Build `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*كتابة /i,(m,n,t)=>`Day ${n} · ${t}: Write `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*ربط /i,(m,n,t)=>`Day ${n} · ${t}: Connect `],
    [/^اليوم\s*(\d+)\s*\(([^)]+)\):\s*استكشاف /i,(m,n,t)=>`Day ${n} · ${t}: Explore `],
  ];
  function marketingText(raw){
    if(!en())return raw;
    let [main,ref]=String(raw).split(' | '); ref=(ref||'').replace(/^🔍\s*/,'');
    if(main.startsWith('المشروع الموحد')) return `Unified project — ${main.replace(/^المشروع الموحد\s*\([^)]*\):\s*/,'')
      .replaceAll('ساعتين ونص','2.5 hours').replaceAll('ساعة','hour').replaceAll('نص ساعة','30 minutes').replaceAll('دقيقة','minutes')}`;
    if(main.startsWith('تحدي الأسبوع')) return `Weekly challenge — ${main.replace(/^تحدي الأسبوع\s*\([^)]*\):\s*/,'').replaceAll('ساعة','hour').replaceAll('نص ساعة','30 minutes')}`;
    if(main.startsWith('تحليل منافس')) return `Competitor analysis — ${main.replace(/^تحليل منافس\s*\([^)]*\):\s*/,'').replaceAll('نص ساعة','30 minutes')}`;
    if(main.startsWith('البورتفوليو')) return `Portfolio — ${main.replace(/^البورتفوليو\s*\([^)]*\):\s*/,'').replaceAll('نص ساعة','30 minutes')}`;
    if(main.startsWith('اختبار')) return `Test — ${main.replace(/^اختبار\s*\([^)]*\):\s*/,'').replaceAll('نص ساعة','30 minutes')}`;
    if(main==='اليوم 7: إجازة تامة!') return 'Day 7 · Full rest.';
    for(const [rx,fn] of replacements){const m=main.match(rx);if(m){main=main.replace(rx,fn);break;}}
