...
function applyLanguage(){
 ...
 document.documentElement.style.colorScheme=state.theme==='paper'?'light':'dark';
 const meta=document.querySelector('meta[name="theme-color"]');
 if(meta)meta.content=getComputedStyle(document.body).getPropertyValue('--bg').trim()||'#0b0f12';
...
function translateText(t){let s=t;const exact=I18N[s.trim()];if(exact)return exact;return s.replaceAll('السبت','Saturday').replaceAll('الأحد','Sunday').replaceAll('الاثنين','Monday').replaceAll('الثلاثاء','Tuesday').replaceAll('الأربعاء','Wednesday').replaceAll('الخميس','Thursday').replaceAll('الجمعة','Friday');}
...
function renderHome(){
 ...
 <div class="stats-grid">
   <div class="stat-card spatial-surface"><small>${T.today}</small><strong>${done}/${total}</strong></div>
   <div class="stat-card spatial-surface"><small>${T.remaining}</small><strong>${remaining}</strong></div>
   <div class="stat-card spatial-surface"><small>${en?'Marketing':'التسويق'}</small><strong>${state.weekly.marketingHours||0}${T.hours}</strong></div>
   <div class="stat-card spatial-surface"><small>${T.target}</small><strong>12${T.hours}</strong></div>
 </div>
 ${smart()}
 <div class="section-title" id="todayTasks"><div><h2>${T.execute}</h2><p>${esc(en?translateText(day):day)} · ${T.ess}.</p></div></div>
 <div class="bento">
   <section class="card tall spatial-surface"><h3>${T.list}</h3>${taskHTML(items)}</section>
   <section class="card spatial-surface"><h3>${T.next}</h3><div class="tasks">
...
function renderQuran(){
 ...
 <div class="grid grid-2">
  <section class="section-box spatial-surface">
   <h3>مراجعة اليوم</h3>
   <div class="task-item ${tChecked('quran')?'done':''}" data-task-id="quran"><input type="checkbox" id="quranToday" ${tChecked('quran')?'checked':''} onchange="toggleToday('quran')"><label class="task-text" for="quranToday">${state.lang==='en'?'Reviewed memorized Qur’an today — Juz ‘Amma / Tabarak / older memorized portions':'راجعت المحفوظ اليوم — عمّ / تبارك / القديم'}</label><span class="mihrab-badge badge-spirit">${state.lang==='en'?'Core':'ورد'}</span></div>
  ...
  <section class="section-box spatial-surface quran-bridge">
   ...
   <div class="bridge-actions"><a class="btn primary holographic-sub" href="${RAFIQ_URL}" target="_blank" rel="noopener">فتح رفيق ↗</a><button class="btn holographic-sub ${open?'active':''}" onclick="toggleRafiqFrame()">${open?'إخفاء داخل اللوحة':'عرض داخل اللوحة'}</button></div>
  </section>
 </div>
 ${open?`<div class="section-title"><div><h2>✦ رفيق القرآن</h2><p>يُحمّل الإطار فقط عند طلبه حتى تظل اللوحة سريعة وخفيفة.</p></div><span class="badge">Cloudflare</span></div><div class="iframe-wrap spatial-surface"><div class="iframe-head"><b>رفيق القرآن</b><div style="display:flex;gap:7px;align-items:center"><span class="badge">Live</span><button class="icon-btn" onclick="toggleRafiqFrame()" aria-label="إغلاق">×</button></div></div><iframe id="rafiqFrame" title="رفيق القرآن داخل Mihrab" src="${RAFIQ_URL}" loading="lazy" allow="autoplay; fullscreen" sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-modals"></iframe></div>`:''}`;
}

function renderCourses(){return `<div class="section-title"><div><h2>📚 الكورسات والأنظمة المساندة</h2><p>مساندة لا تسبق الأساسيات.</p></div></div><div class="grid grid-3"><section class="section-box spatial-surface"><h3>🧠 Anki</h3><p>المراجعة المستحقة أولًا. الجديد بميزانية ثابتة. أماكنه المفضلة: المواصلات، المصلى، والفواصل.</p></section><section class="section-box spatial-surface"><h3>🚀 McKinsey Forward</h3><p>حوالي ساعتين أسبوعيًا. أول ما ينكمش عند ضغط الدراسة.</p><label class="task ${state.weekly.mckinsey?'done':''}"><input type="checkbox" ${state.weekly.mckinsey?'checked':''} onchange="state.weekly.mckinsey=this.checked;save();renderAll()"><span>أنجزت نصيب الأسبوع</span></label></section><section class="section-box spatial-surface"><h3>💊 The Pharmacist's Guide to Dose Calculations</h3><p>2:41 ساعة إجماليًا · جلسات قصيرة 10–15 دقيقة تقريبًا. أيضًا من أول الأشياء التي يمكن تأجيلها عند الضغط.</p><label class="task ${state.weekly.dose?'done':''}"><input type="checkbox" ${state.weekly.dose?'checked':''} onchange="state.weekly.dose=this.checked;save();renderAll()"><span>أنجزت نصيب الأسبوع</span></label></section></div><div class="grid grid-2" style="margin-top:12px"><section class="section-box spatial-surface"><h3>🎓 HubSpot / Google</h3><p>20–30 دقيقة وقت الفراغ، بالتوازي مع المعسكر، بدون إعادة شرح ما تدرسه بالفعل.</p><label class="task ${state.weekly.cert?'done':''}"><input type="checkbox" ${state.weekly.cert?'checked':''} onchange="state.weekly.cert=this.checked;save();renderAll()"><span>أنجزت نصيب الشهادة الموازية</span></label></section><section class="section-box spatial-surface"><h3>📗 EasyPeasy</h3><p>يظل موجودًا كقراءة خفيفة داخل اليوم، ولا يضخم الخطة الرئيسية.</p></section></div>`}
...
(function(){
  ...
  function ensureState(){
    state.today ||= {}; state.plan ||= {};
    state.weekly={marketingHours:0,mckinsey:false,dose:false,review:false,rating:'',cert:false,...(state.weekly||{})};
    state.weekDayTypes ||= {}; state.settings={lowPower:false,...(state.settings||{})};
    state.inbox ||= []; state.library ||= [];
  ...
  function render(id){
    const renderer={home:renderHome,marketing:renderMarketing,shari:renderShari,quran:renderQuran,courses:renderCourses,system:renderSystemBase}[id];
  ...
  // Spatial View Transition
  function showView(id){
    $$('.view').forEach(v=>{
      const on=v.id==='view-'+id;
      if (on) {
        v.hidden = false;
        // Trigger CSS spatial entrance
  ...
  function route(id,{scroll=true,push=true}={}){
    ...
    catch(err){console.error('[Mihrab]',err);}
  ...
  function startFocus(id){
    ...
    shell.innerHTML=`<div class="focus-kicker">${state.lang==='en'?'MIHRAB FOCUS':'تركيز مِحْرَاب'}</div><h2>${esc(item.label)}</h2><div class="focus-timer">25:00</div><div class="focus-actions"><button class="btn primary holographic-sub" onclick="finishFocus('${esc(item.id)}');closeFocus()">${state.lang==='en'?'Mark done':'تم'}</button><button class="btn" onclick="extendFocus(5)">+5m</button><button class="btn" onclick="closeFocus()">${state.lang==='en'?'Exit':'خروج'}</button></div>`;
  ...
  function ensureModals(){if($('#mihrabOverlay'))return;document.body.insertAdjacentHTML('beforeend',`<div class="mihrab-overlay" id="mihrabOverlay" role="dialog" aria-modal="true"><div class="mihrab-modal spatial-surface" id="mihrabModal"></div></div><div class="focus-mode" id="focusMode"><div class="focus-shell spatial-surface" id="focusShell"></div></div>`)}
  ...
  window.filterCommands=q=>{const list=$('#commandList');if(!list)return;const en=state.lang==='en';const items=[...NAV.map(x=>[x[0],en?x[3]:x[2],()=>navigate(x[0])]),['focus',en?'Start Focus':'ابدأ التركيز',startFocus],['smart15','15m',()=>smartTime(15)],['smart30','30m',()=>smartTime(30)]].filter(x=>(x[0]+' '+x[1]).toLowerCase().includes((q||'').toLowerCase()));list.innerHTML=items.map(([id,label])=>`<button class="command-item" data-command="${id}"><span>${esc(label)}</span><small>↵</small></button>`).join('')||`<div class="note">${en?'No command found.':'مش لاقي الأمر ده.'}</div>`};
  document.addEventListener('click',e=>{const c=e.target.closest('[data-command]');if(!c)return;const id=c.dataset.command;closeModal();if(VIEWS.includes(id))return navigate(id);if(id==='focus')return startFocus();if(id==='smart15')return smartTime(15);if(id==='smart30')return smartTime(30);});

  // Central Pointer Engine - Calculates physical variables for CSS Spatial Material
  function bindThemePointer(){
    if(!window.matchMedia || !matchMedia('(pointer:fine)').matches) return;
    let raf=0, x=.5, y=.5, tx=x, ty=y;
    const tick=()=>{
      raf=0;
      ...
      // Update ambient light positions
      ...
      // Calculate normalized -1 to +1 coordinates for physical 3D tilt
      document.documentElement.style.setProperty('--mx', ((x - 0.5) * 2).toFixed(4));
      document.documentElement.style.setProperty('--my', ((y - 0.5) * 2).toFixed(4));
      
      if(Math.abs(tx-x)+Math.abs(ty-y)>.001) raf=requestAnimationFrame(tick);
    };
