...
(function(){
  ...
  function taskObjects(){
    ...
    state.library.filter(x=>x.status==='active'&&x.days?.includes(todayName())).forEach(x=>items.push({id:x.id,label:state.lang==='en'&&x.titleEn?x.titleEn:x.title,duration:x.duration||15,priority:x.core?'core':x.important?'important':'optional',done:!!state.today[x.id],custom:true}));
    return items;
  }
  function smartTime(min){
    ...
    const en=state.lang==='en';
    if(!candidates.length){return showNotice(en?`Nothing useful fits ${min} minutes.`:`مفيش حاجة مفيدة مناسبة لـ ${min} دقيقة.`)}
    openModal(`<div class="mihrab-modal-head"><div class="modal-title-wrap"><b>${en?`Good fits for ${min}m`:`المناسب لـ ${min} دقيقة`}</b><small>${en?'Pick one and start focus immediately.':'اختار واحدة وابدأ التركيز عليها فورًا.'}</small></div><button class="mihrab-close" onclick="closeMihrabModal()">×</button></div><div class="command-list smart-list">${candidates.map(x=>`<button class="command-item smart-choice" onclick="startFocus('${esc(x.id)}')"><span><b>${esc(x.label)}</b><small>${x.duration}m · ${x.priority==='core'?(en?'Core':'أساسي'):x.priority==='important'?(en?'Important':'مهم'):(en?'Optional':'اختياري')}</small></span><small>↵</small></button>`).join('')}</div>`);
  }
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(e.key==='Escape'){if($('#focusMode')?.classList.contains('open'))closeFocus();else closeModal();}
    if(e.key==='Enter'&&$('#commandInput')&&document.activeElement===$('#commandInput')){const first=$('#commandList [data-command]');first?.click();}
  });
  ...
  function startFocus(id){
    const item=(id&&taskObjects().find(x=>x.id===id&&!x.done))||nowNextLater()[0]; if(!item){showNotice(state.lang==='en'?'Core is complete.':'الأساسيات خلصت.');return;}
  ...
  function ensureModals(){
    ...
    $('#mihrabOverlay').addEventListener('click',e=>{if(e.target.id==='mihrabOverlay')closeModal()});
    $('#focusMode').addEventListener('click',e=>{if(e.target.id==='focusMode')closeFocus()});
  }
  function closeModal(){$('#mihrabOverlay')?.classList.remove('open')}
  function showNotice(message,title){const en=state.lang==='en';openModal(`<div class="mihrab-modal-head"><div class="modal-title-wrap"><b>${esc(title|| (en?'Mihrab':'مِحْرَاب'))}</b></div><button class="mihrab-close" onclick="closeMihrabModal()">×</button></div><div class="modal-body notice-body"><p>${esc(message)}</p><div class="modal-actions"><button class="btn primary" onclick="closeMihrabModal()">${en?'Done':'تمام'}</button></div></div>`)}
  function showConfirm(message,onConfirm){const en=state.lang==='en';const token='confirm_'+Date.now().toString(36);window[token]=()=>{try{onConfirm()}finally{delete window[token];closeModal()}};openModal(`<div class="mihrab-modal-head"><div class="modal-title-wrap"><b>${en?'Confirm action':'تأكيد الإجراء'}</b></div><button class="mihrab-close" onclick="closeMihrabModal()">×</button></div><div class="modal-body notice-body"><p>${esc(message)}</p><div class="modal-actions"><button class="btn" onclick="closeMihrabModal()">${en?'Cancel':'إلغاء'}</button><button class="btn primary" onclick="${token}()">${en?'Confirm':'تأكيد'}</button></div></div>`)}
  window.showNotice=showNotice;window.showConfirm=showConfirm;
  ...
  window.archiveContent=id=>{const x=state.library.find(i=>i.id===id);if(!x)return;showConfirm(state.lang==='en'?'Archive this track?':'توقيف هذا المسار مؤقتًا؟',()=>{x.status='paused';save();rerender()})};
  window.activateContent=id=>{const x=state.library.find(i=>i.id===id);if(x){x.status='active';save();rerender()}};
  ...
  window.openTrackDetails=id=>{const x=state.library.find(i=>i.id===id);if(!x)return;const title=state.lang==='en'&&x.titleEn?x.titleEn:x.title;const d=x.days?.length?x.days.join(' · '):(state.lang==='en'?'Unscheduled':'غير مجدول');openModal(`<div class="mihrab-modal-head"><b>${esc(title)}</b><button class="mihrab-close" onclick="closeMihrabModal()">×</button></div><div class="modal-body"><div class="detail-grid"><div><small>Status</small><b>${esc(x.status)}</b></div><div><small>Duration</small><b>${x.duration||30} min</b></div><div><small>Schedule</small><b>${esc(d)}</b></div><div><small>Progress</small><b>${x.sessions?`${x.completedSessions||0}/${x.sessions}`:'Open'}</b></div></div><div class="modal-actions"><button class="btn" onclick="editContent('${x.id}')">${state.lang==='en'?'Edit':'تعديل'}</button><button class="btn primary" onclick="closeMihrabModal()">${state.lang==='en'?'Close':'إغلاق'}</button></div></div>`)};

  function systemExtras(){
    const en=state.lang==='en';
    const inbox=state.inbox.filter(x=>x.status==='inbox');
    const active=state.library.filter(x=>x.status==='active');
    const paused=state.library.filter(x=>x.status==='paused');
    const card=x=>`<div class="library-item"><div class="grow"><button class="library-title-btn" onclick="openTrackDetails('${x.id}')"><b>${esc(en&&x.titleEn?x.titleEn:x.title)}</b><span>↗</span></button><small>${esc(x.category)} · ${x.duration||30} min · ${(x.days||[]).join(' · ')|| (en?'Unscheduled':'غير مجدول')}</small></div><div class="library-meta"><span class="library-status ${x.status==='active'?'active':'paused'}">${x.status==='active'?(en?'Active':'نشط'):(en?'Paused':'متوقف')}</span>${x.status==='active'?`<button class="tiny-action" onclick="editContent('${x.id}')">${en?'Edit':'تعديل'}</button><button class="tiny-action" onclick="archiveContent('${x.id}')">${en?'Pause':'إيقاف'}</button>`:`<button class="tiny-action" onclick="activateContent('${x.id}')">${en?'Activate':'تفعيل'}</button>`}</div></div>`;
    return `<section class="section-box lifecycle-card"><div class="section-title"><div><h2>${en?'Long-run system':'النظام على المدى الطويل'}</h2><p>${en?'The plan stays yours; content can evolve without editing code.':'الخطة ملكك، والمحتوى يتغير من غير لمس الكود.'}</p></div><button class="btn primary" onclick="addContent()">＋ ${en?'Add track':'إضافة مسار'}</button></div><div class="library-list">${active.map(card).join('')||`<div class="note">${en?'No active tracks.':'لا توجد مسارات نشطة.'}</div>`}</div>${paused.length?`<div class="library-archive"><div class="section-title compact"><div><h3>${en?'Paused / archived':'المسارات المتوقفة'}</h3><p>${en?'Nothing is deleted; paused tracks can return any time.':'مفيش حذف؛ أي مسار متوقف تقدر ترجّعه في أي وقت.'}</p></div><span class="badge">${paused.length}</span></div><div class="library-list">${paused.map(card).join('')}</div></div>`:''}</section><div class="grid grid-2 lifecycle-grid"><section class="section-box"><h3>${en?'Inbox':'صندوق الوارد'}</h3><p class="muted">${en?'Quick ideas land here first.':'أي فكرة سريعة تدخل هنا الأول.'}</p>${inbox.length?inbox.slice(0,8).map(x=>`<div class="inbox-item"><div class="grow"><b>${esc(x.text)}</b><small>${x.duration}m</small></div><button class="tiny-action" onclick="completeInbox('${x.id}')">${en?'Done':'تم'}</button><button class="tiny-action" onclick="deleteInbox('${x.id}')">×</button></div>`).join(''):`<div class="note">${en?'Inbox is clear.':'صندوق الوارد فاضي.'}</div>`}<button class="btn" onclick="openQuickCapture()">＋ ${en?'Quick capture':'إضافة سريعة'}</button></section><section class="section-box"><h3>${en?'Backup & restore':'النسخ الاحتياطي'}</h3><p class="muted">${en?'Keep a JSON backup before moving devices or making major changes.':'خُد نسخة JSON قبل نقل الجهاز أو أي تغيير كبير.'}</p><div class="modal-actions"><button class="btn primary" onclick="exportMihrab()">${en?'Export JSON':'تصدير JSON'}</button><button class="btn" onclick="openImport()">${en?'Import':'استيراد'}</button></div></section></div>`;
  }
  ...
  try{
    ...
    bindThemePointer();
    document.documentElement.dataset.mihrabReady='true';
  }catch(err){
    console.error('[Mihrab boot]',err);
    document.documentElement.dataset.mihrabReady='false';
    const host=document.querySelector('#view-home');
    if(host) host.innerHTML='<section class="section-box error-box"><h2>مِحْرَاب</h2><p class="muted">تعذر تشغيل الواجهة. أعد تحميل الصفحة.</p></section>';
  }
})();
...
window.installPWA=async()=>{if(deferredInstallPrompt){await deferredInstallPrompt.prompt();await deferredInstallPrompt.userChoice;deferredInstallPrompt=null;return;}showNotice(state.lang==='en'?'Use the browser menu and choose Install app / Add to Home Screen.':'من قائمة المتصفح اختر «تثبيت التطبيق» أو «إضافة إلى الشاشة الرئيسية».')};
